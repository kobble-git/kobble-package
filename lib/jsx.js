...
export let FlowComponentJsx = (self, cards) => (
  <div className="flow-layout-container">
        {_.map(cards, (card, i) => {
          return (<div className="flow-card-container">
            <FlowCard card={card}/>
          </div>)
        })}
  </div>
...
export let FlowCardJsx = (self, card) => (
  <Card expanded={card.expanded}>
    ...
    <CardText expandable={true}>
      <TextField errorText={card.projectErrorText} value={card.projectName} hintText="Enter Project Name" fullWidth={true} onChange={self.handleProjectChange}/>
      <TextField errorText={card.filenameErrorText} value={card.flowName} hintText="Enter Pipeline Name" fullWidth={true} onChange={self.handleFilenameChange}/>
    </CardText>
    <CardActions>
