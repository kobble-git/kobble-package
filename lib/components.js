  ...
  async installFlow(flow) {
    try {
      ...
      if (!flow.project || !flow.filename) {
        ...
        let dir = path.join(kobbleProjectsDir, flow.project);
        if (!await exists(dir))
          await createDir(dir);
        flow.expanded = false;
        await this.copyFlow(flow, dir, flow.filename);
